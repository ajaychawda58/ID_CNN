...
def parse_args():
    ...
    parser.add_argument('--augment', dest='augment', help='augmented dataset', default='image_2', type=str)
    parser.add_argument('--backbone', dest='backbone', help='test data model', default='vgg16', type=str)
...
if args.test=='pascal_voc':
	model_path = os.path.join(root, "trained_model", "pascal_voc", "faster_rcnn", "vgg19")
	save_path = os.path.join("/scratch", "chawda", "trained_model", "pascal_voc", "faster_rcnn", "vgg19")
	num_classes=20
	...
	model_path = os.path.join(root, "trained_model", "coco", "faster_rcnn", "vgg19")
	save_path = os.path.join("/scratch","chawda", "trained_model", "coco", "faster_rcnn", "vgg19")
	num_classes=91
...
if args.backbone == 'vgg16':
	...
	save_path = os.path.join("/scratch", "chawda", "trained_model", "kitti", "retinanet", "vgg16")
else:
	backbone = vgg19(pretrained=False).features
	model_path = os.path.join(root, "trained_model", "kitti", "retinanet", "vgg19")
	save_path = os.path.join("/scratch", "chawda", "trained_model", "kitti", "retinanet", "vgg19")
...
for i, data in tqdm(enumerate(dataloader, 0)):
	...
	if args.backbone == 'vgg16':
		out0,out1,out2,out3,out4,out5,out6,out7,out8,out9 = extract_vgg16(model, image)
	else:
		out0,out1,out2,out3,out4,out5,out6,out7,out8,out9 = extract_vgg19(model, image)
...
del Out0,Out1,Out2,Out3,Out4,Out5,Out6,Out7,Out8,Out9
