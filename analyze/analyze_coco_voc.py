...
def parse_args():
    ...
    parser.add_argument('--testdata', dest='test', help='test data model', default='coco', type=str)
...
if args.test=='pascal_voc':
	model_path = os.path.join(root, "trained_model", "coco", "faster_rcnn", "vgg16")
	save_path = os.path.join("/scratch", "chawda", "trained_model", "pascal_voc", "faster_rcnn", "vgg16", "coco")
	num_classes=91
	coco = 1
else:
	model_path = os.path.join(root, "trained_model", "pascal_voc", "faster_rcnn", "vgg16")
	save_path = os.path.join("/scratch","chawda", "trained_model", "coco", "faster_rcnn", "vgg16", "voc")
	num_classes=20
	coco = 0
...
def get_featmap_and_size(model, image):
	...
	out4 = model.backbone[23](F.relu(model.backbone[21](F.relu(model.backbone[19](F.relu(model.backbone[17](out3)))))))
	x = OrderedDict()
	x['0'] = F.relu(model.backbone[24](out4))
	if dataset == 'kitti':
