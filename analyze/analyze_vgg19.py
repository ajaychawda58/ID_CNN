...
def parse_args():
    ...
    parser.add_argument('--augment', dest='augment', help='augmented dataset', default='test2017', type=str)
...
if args.test=='pascal_voc':
	...
	data = torch.utils.data.Subset(data, indices[:2000])
elif args.test == 'coco':
	...
	model_path = os.path.join(root, "trained_model", "kitti", "faster_rcnn", "vgg19")
	save_path = os.path.join("/scratch", "chawda", "trained_model", "kitti", "faster_rcnn", "vgg19")
	num_classes = 10
...
def get_featmap_and_size(model, image):
	...
	out4 = model.backbone[27](F.relu(model.backbone[25](F.relu(model.backbone[23](F.relu(model.backbone[21](F.relu(model.backbone[19](out3)))))))))
	x = OrderedDict()
	x['0'] = F.relu(model.backbone[28](out4))
	if args.test == 'kitti':
